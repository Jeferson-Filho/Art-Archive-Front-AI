...
| 02  | Requisitos Funcionais e Não Funcionais                        | Especifica o que o sistema deve fazer (funcional) e com que qualidade (não funcional — performance, segurança, usabilidade).   | ✅ Concluído                         |
| 03  | Casos de Uso (atores e funcionalidades, Gherkin)              | Descreve interações entre usuário e sistema em cenários concretos e testáveis.                                                 | ✅ Concluído                         |
| 04  | Arquitetura do Sistema (componentes, implantação, sequência)  | Mostra como o sistema é estruturado tecnicamente: módulos, infraestrutura, fluxo de chamadas.                                  | ✅ Concluído                         |
| 05  | Banco de Dados (DER, esquema relacional, dicionário de dados) | Modela como os dados são armazenados e relacionados.                                                                           | ✅ Concluído                         |
| 06  | Diagrama de Fluxo de Persistência                             | Detalha especificamente a lógica de "verificar antes de gerar" — o mecanismo de cache do Insight Card.                         | ✅ Concluído                         |
| 07  | Especificação de Interface (API Contract)                     | Define contratos formais de cada endpoint: request, response, erros.                                                           | ✅ Concluído                         |
| 08  | Navegação e Fluxo (diagrama de navegabilidade)                | Mapeia as telas e transições da experiência do usuário.                                                                        | ✅ Concluído                         |
| 09  | Pipeline de IA (Insight Card e Alt Text)                      | Detalha o fluxo técnico específico de geração de conteúdo por IA, do input ao output persistido.                               | ✅ Concluído                         |
| 10  | Design do Card                                                | Wireframe/mockup visual do Insight Card, incluindo estados (loading, gerado, erro).                                            | 🖼️ Pronto — arquivo `.png` a incluir |
| 11  | Documento de Prompt Engineering                               | Documenta a estratégia, versionamento e evolução dos prompts usados.                                                           | ✅ Concluído                         |
...
> **Nota sobre o doc 10:** diferente dos demais, este documento é uma imagem (`.png`), não um arquivo Markdown. O design já foi produzido externamente e está com um terceiro; falta apenas trazer o arquivo para `documentation/specs/10-design-card.png`. Para fins de planejamento (doc 99), ele é considerado pronto — nenhuma fase depende da criação do design, apenas da chegada do arquivo.
